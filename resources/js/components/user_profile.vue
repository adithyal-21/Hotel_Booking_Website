<template>
    <v-app>
        <div>
            <v-toolbar color="#1A237E" dark>
                <router-link to="/user_dash" style="text-decoration:none">
                    <v-btn icon>
                        <v-icon style="font-size:23px">fa fa-arrow-left</v-icon>
                    </v-btn>
                </router-link>
                <v-breadcrumbs :items="items">
                    <template v-slot:divider>
                        <v-icon>mdi-chevron-right</v-icon>
                    </template>
                </v-breadcrumbs>
                <v-spacer></v-spacer>

                <v-menu bottom left>
                    <template v-slot:activator="{ on, attrs }">
                        <v-btn icon color="white" v-bind="attrs" v-on="on">
                            <v-icon>mdi-dots-vertical</v-icon>
                        </v-btn>
                    </template>

                    <v-list>
                        <v-list-item>
                            <router-link
                                :to="{
                                    name: 'user_image',
                                    params: { id: data.id }
                                }"
                                style="text-decoration:none;color:inherit"
                            >
                                <v-list-item-title
                                    >Upload Image</v-list-item-title
                                >
                            </router-link>
                        </v-list-item>
                    </v-list>
                </v-menu>
            </v-toolbar>
            <br />
            <div class="conatiner">
                <div class="row justify-content-center">
                    <v-card id="card" width="550" hover shaped>
                        <div v-if="data.image == null">
                            <v-img
                                src="profile.jpg"
                                style="border-top-left-radius:25px;"
                                height="300px"
                                id="image1"
                            >
                            </v-img>
                        </div>
                        <div v-else>
                            <v-img
                                :src="'/uploads/' + data.image"
                                height="300px"
                                style="border-top-left-radius:25px"
                            >
                            </v-img>
                        </div>
                        <h1 style="margin-top:20px">
                            {{ data.name }}
                        </h1>
                        <h1>{{ data.phone }}</h1>
                        <h1>{{ data.email }}</h1>

                        <h3><span>UPDATE HERE</span></h3>
                        <router-link
                            :to="{
                                name: 'user_update',
                                params: { id: data.id }
                            }"
                            style="text-decoration:none;color:inherit"
                        >
                            <div style="text-align:center;margin-top:20px">
                                <v-btn
                                    rounded
                                    color="indigo"
                                    width="200px"
                                    dark
                                >
                                    Update
                                </v-btn>

                                <br /><br />
                            </div>
                        </router-link>
                    </v-card>
                </div>
            </div></div
    ></v-app>
</template>
<script>
export default {
    data() {
        return {
            items: [
                {
                    text: "𝗗𝗔𝗦𝗛𝗕𝗢𝗔𝗥𝗗",
                    disabled: false
                },
                {
                    text: "𝗣𝗥𝗢𝗙𝗜𝗟𝗘",
                    disabled: false
                }
            ],
            data: {}
        };
    },
    methods: {
        getuserProfile(id) {
            axios
                .get(`http://127.0.0.1:8000/api/getUserProfile/${id}`)
                .then(Response => {
                    this.data = Response.data.data1;
                });
        }
    },
    created() {
        this.getuserProfile(sessionStorage.getItem("userid"));
    },
    mounted() {
        var email = sessionStorage.getItem("useremail");
        if (email == null) {
            this.$router.push({ path: "Userlog" });
        }
    }
};
</script>
<style scoped>
h1 {
    text-align: center;
    font-family: Bahnschrift SemiBold;
    margin-top: 10px;
    font-size: 28px;
    color: #455a64;
}
h3 {
    width: 100%;
    text-align: center;
    border-bottom: 1px solid lightgray;
    line-height: 0.1em;
    margin: 35px 0 30px;
    font-family: Bahnschrift SemiBold;
    font-size: 18px;
}

h3 span {
    background: #fff;
    padding: 0 10px;
    color: #5c6bc0;
}
#card {
    margin-top: 20px;
    border-bottom: solid 25px #3949ab;
    border-right: solid 25px #3949ab;
}

table th {
    font-family: Bahnschrift SemiBold;
    font-size: 18px;
    border-top: none;
    text-transform: capitalize;
}
table td {
    border-top: none;
    font-family: Bahnschrift;
    font-weight: lighter;
    font-size: 16px;
    text-transform: capitalize;
}
table tr {
    height: 60px;
}

@media (min-width: 300px) and (max-width: 350px) {
    #card {
        border-bottom: solid 20px #3949ab;
        border-right: solid 20px #3949ab;
    }
}
/*meduim mobile phones*/
@media (min-width: 350px) and (max-width: 400px) {
    #card {
        border-bottom: solid 20px #3949ab;
        border-right: solid 20px #3949ab;
    }
}
/*large mobile phones*/
@media (min-width: 400px) and (max-width: 500px) {
}
/*tablet*/

/*tablet mode*/
@media (min-width: 600px) and (max-width: 800px) {
    #card {
        width: 900px;
    }
}
@media (min-width: 800px) and (max-width: 1050px) {
}
</style>
